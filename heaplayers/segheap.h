...
#include "bitstring.h"
  ...
  namespace HL {
	...
	: memoryHeld (0),
      ...
      {
      //fprintf(stderr, "segheap free: memoryHeld %p sizeof(LittleHeap) %d NumBins %d sizeof(segHeap) %d\n", &memoryHeld, sizeof(LittleHeap), NumBins, sizeof(SegHeap));
  ...
  //fprintf(stderr, "LTP: SEGHEAP: sz %d maxObjectSize %d\n", sz, maxObjectSize);
	if (sz > maxObjectSize) {
	  goto GET_MEMORY;
	}
...
namespace HL {
  ...
  public:
	...
	if (!ptr) {
    //fprintf(stderr, "LTP: strictSegHeap malloc ptr %p little failed\n", ptr);
	  ptr = super::bigheap.malloc (objectSize);
    //fprintf(stderr, "LTP: strictSegHeap malloc ptr %p after\n", ptr);
	}
      } else {
	...
	super::myLittleHeap[objectSizeClass].free (ptr);
	  //fprintf (stderr, "myLittleHeap pointer %p pointing to %lx\n", (void *)&super::myLittleHeap[objectSizeClass], *((unsigned long *)&super::myLittleHeap[objectSizeClass]));
