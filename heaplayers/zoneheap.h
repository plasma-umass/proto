...
namespace HL {
  ...
  public:
    ...
    inline void * malloc (size_t sz) {
      //fprintf(stderr, "LTP: zoneheap malloc\n");
  ...
  private:
    ...
    inline void * zoneMalloc (size_t sz) {
      ...
      //fprintf(stderr, "LTP: zoneheap malloc not in slowMalloc\n");
    ...
    void * slowMalloc (size_t sz) {
      ...
      // We don't want to add one more page here.
      void * buf = Super::malloc(allocSize);
      if (buf == NULL) {
	      return NULL;
      }
      //fprintf(stderr, "LTP: zoneheap malloc in slowMalloc, allocSize %x  buf %p\n", allocSize, buf);
      //fprintf(stderr, "LTP: zoneheap malloc in slowMalloc after super\n");
      _currentArena = new (buf) Arena (allocSize- PAGE_SIZE);
	...
	: _remaining (sz),
	  _nextArena (NULL),
	  _arenaSpace ((char *) ((intptr_t)this + PAGE_SIZE))
      {}
      inline void * malloc (size_t sz) {
	if (sz > _remaining) {
	  return NULL;
